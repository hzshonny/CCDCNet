...
class Pix2PixModel(torch.nn.Module):
    ...
    def __init__(self, opt):
        ...
        if opt.isTrain:
            ...
            self.vggnet_fix.load_state_dict(torch.load('/data1/yaojia/matecoco/models/vgg19_conv.pth'))
    ...
    def compute_generator_loss(self, input_label, input_semantics, real_image, ref_label=None, ref_semantics=None,
        ...
        """gan loss"""
        '''-----------------------------------------------'''
        # print( generate_out['fake_image'].shape) # torch.Size([4, 3, 256, 256])
    ...
    def generate_fake(self, input_semantics, real_image, ref_semantics=None, ref_image=None, self_ref=None):
        ...
        with autocast(enabled=self.opt.amp):
            '''---------------------------------------------------------------'''
            # print(input_semantics.shape) # torch.Size([4, 2, 256, 256])
            # print(ref_semantics.shape) # torch.Size([4, 2, 256, 256])
            # print(real_image.shape) # torch.Size([4, 3, 256, 256])
