        ...
        if opt.save_per_img:
            try:
                for it in range(imgs_num):
                    ...
                    filename = os.path.splitext(name)[0]  # 文件扩展名
                    filetype = '.png'
                    save_name=os.path.join(save_root, 'label', filename + filetype)
            ...
            try:
                ...
                print(err)
#############--PONO --PONO_C
'''python test.py --name ade20k --dataset_mode ade20k  --amp --batchSize 4 
--load_size 256 --crop_size 256 --dataroot ./dataset/knife
--no_flip --which_epoch latest --use_attention --PONO --PONO_C --save_per_img  # --maskmix # 
'''
