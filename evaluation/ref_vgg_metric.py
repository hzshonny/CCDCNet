...
class Dataset_(torch.utils.data.Dataset):
    def __init__(self, folder, dataset_mode):
        ...
        if dataset_mode == 'ade20k':
            # self.label_folder = './Dataset/ADEChallengeData2016/images/validation'
            # self.ref_folder = './Dataset/ADEChallengeData2016/images/training'
            # self.ref_label_folder = './Dataset/ADEChallengeData2016/images/training'
            # ref_pair_txt = './data/ade20k_ref_test_from_train.txt'
            self.label_folder = './output/label'
            self.ref_folder = './output/ref'
            self.ref_label_folder ='./output/ref_label'
            imgs_name = os.listdir(self.ref_folder)
            imgs_name = [it for it in imgs_name if it[-4:] == '.jpg']
        elif 'celebahq' in dataset_mode:
            self.label_folder = '../../../CelebAMask-HQ/all_parts_except_glasses'
            self.ref_folder = '../../../CelebAMask-HQ/CelebA-HQ-img'
            self.ref_label_folder = '../../..//CelebAMask-HQ/all_parts_except_glasses'
            ref_pair_txt = '../../..//CelebAMask-HQ/celebahq_ref_test.txt'
            ...
            self.label_folder = '../../..//Metfaces/edge'
            self.ref_folder = '../../..//Metfaces/metfaces/metface'
            self.ref_label_folder = '../../..//Metfaces/edge'
            ref_pair_txt = '../../../Metfaces/cocosnet/metface_ref_test.txt'
        elif dataset_mode == "aahq":
            self.label_folder = '../../..//aahq/aahq_edge'
            self.ref_folder = '../../../aahq/aahq_data'
            self.ref_label_folder = '../../..//aahq/aahq_edge'
            ref_pair_txt = '../../../aahq/dict/aahq_ref_test.txt'
        ...
        for i in range(len(imgs_name)):
            ...
            if not os.path.exists(os.path.join(self.folder, name)):
                ...
                ref_label_paths.append(os.path.join(self.ref_label_folder, self.tolabel_path(imgs_name[i])))
                ref_img_paths.append(os.path.join(self.ref_folder, imgs_name[i]))
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(self.gen_img_paths[index]).convert('RGB')
...
if __name__ == '__main__':
    ...
    vgg.load_state_dict(torch.load('./models/vgg19_conv.pth'))
    # vgg.load_state_dict(torch.load('./vgg/vgg19_conv.pth'))
    vgg.cuda()
    for param in vgg.parameters():
        param.requires_grad = False


    fake_path = './output/fake'
    dataset_mode = 'ade20k'
    use_cos = 'cos'
...
#python ref_vgg_metric.py ./output/test_per_img/ade20k_vae_v100 ade20k
'''python ref_vgg_metric.py'''
